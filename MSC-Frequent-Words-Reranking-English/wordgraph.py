...
verb_pos_set = ['VB', 'VBD', 'VBG', 'VBN', 'VBP', 'VBZ']
...
def action(lst_similar_tagged_sentences, lst_stop_words):
    ...
    """ print('Generate available sentences') """
    k = [None] * 500
    tmp_unique_set = set()
    ...
    available_sentences_nodes = sorted(available_sentences_nodes, key=lambda item: item[0], reverse=True)

    compress_result = ''
    try:
        compress_result = (available_sentences_nodes[0][1]).replace(" 'm ", "'m ").replace(" 'll ", "'ll ").replace(" 'd ", "'d ").replace(" n't ", "n't ").replace(" 's ", "'s ")
    except:
        pass
